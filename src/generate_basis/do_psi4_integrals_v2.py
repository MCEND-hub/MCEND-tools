...
from read_params import (
    ...
    read_poly_grid_parameters,
    write_molden_basis_string,
)
from read_params import one_ints_write, two_eri_write
from read_params import read_coord
from gen_coord import gen_ghost_atom_positions, gen_nrgrid_positions_v2, gen_grd_file
...
parser.add_argument(
    "-r",
    "--runtype",
    help="Moving basis. Leave out flag if basis is static (default=static)",
    required=False,
)
parser.add_argument(
    "-b",
    "--basis",
    help="Which basis: cc-pvdz, 6-31G**, etc, quotes are not needed.",
    required=True,
)
parser.add_argument(
    "-f",
    "--formula",
    help="what compound, to load rsp and grid_parameters",
    required=False,
)
parser.add_argument(
    "-coord", "--sample_coordinate", help="sample internal coordinate", required=False
)
parser.add_argument(
    "-mc", "--charge", help="Molecular charge (default=0)", required=False
)
parser.add_argument(
    "-ms", "--multiplicity", help="Molecular multiplicity (default=1)", required=False
)
parser.add_argument(
    "-i",
    "--intsmethod",
    help="use python to write instead of fortran (default=no)",
    required=False,
)
parser.add_argument(
    "-fc",
    "--fortcompiler",
    help="Fortran compiler to use (gfortran or ifort). Option deprecated",
    required=False,
)
...
if args["formula"]:
    ...
    atm, type_list, type_list_total, link_list, value_list = read_coord(
        input_coord_name
    )
...
print(
    "before gen_poly_cartesian", atm, "link_list", link_list, "value_list", value_list
)
# e.g., for HCN
...
cartesian_coordinates = gen_ghost_atom_positions(
    input_rsp_name, atm, link_list, value_list
)
...
for k in np.arange(1, len(cartesian_coordinates) + 1):
    # if this is the first run, write all integrals out
    write_all = bool(k == 1)

    # below variable is never used, commenting it out
    # ISU
    # time_init = time.time()
    geom_str = ""
    geom_str += "nocom \n"
    ...
    for i in range(len(cartesian_coordinates)):
        if i + 1 == k:
            ...
            for m in range(len(atm)):
                geom_str += " {}   0.0  0.0  {: 9.7f}\n".format(
                    atm[m], cartesian_coordinates[i][m][2]
                )
    ...
    T = np.absasarray(mints.ao_kinetic(), order="F")
    nbf = int(np.shape(T)[0])
    ...
    if write_all:
        Vee = np.asarray(mints.ao_eri(), order="F").reshape(nbf, nbf, nbf, nbf)

    print("Writing all integrals")
    H = T + V
    x = np.array(r[0], order="F")
    y = np.array(r[1], order="F")
    z = np.array(r[2], order="F")
    # > write in tridiagonal form
    il1 = np.tril_indices(nbf)

    print("len", len(T), len(H))
    print("shape", np.shape(T), np.shape(H))
    ...
    # > Call fortran program to write files, because python is way too slow at writing to output
        ...
        if write_all:
            ...
            rijindex = np.array(rijindex, order="F", dtype=int)
            rijindex = rijindex.T
            rij = np.array(rij, order="F")
            nrteint = len(rij)
...
# scfguess is not defined, so I'm commenting this out
# ISU
# if runtype == 'moving':
#    scfguess.close()
...
def gen_scf_guess(
    ...
    for i in range(n_dof):
        ...
        # rgrid = linspace(rmin, rmax, nrprimn)*bohr2Ang
        rgrid = np.linspace(rmin[i], rmax, nrprimn[i])  # *bohr2Ang
        ...
        cartesian_coordinates = gen_nrgrid_positions_v2(
            atm, link_list, value_list, rgrid, i
        )
        # cartesian_coordinates = gen_nrgrid_positions_v2(rgrid, i)

        print(
            "cartesian_coordinates!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!",
            len(cartesian_coordinates),
        )
        ...
        for k in np.arange(1, len(cartesian_coordinates) + 1):
            ...
            _ = psi4.core.MintsHelper(wfn.basisset())
